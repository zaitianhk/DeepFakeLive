...
def create_root(start: Callable[[], None], destroy: Callable[[], None]) -> ctk.CTk:
    ...
    donate_label = ctk.CTkLabel(root, text='DeepFakeLive By ZAI TIAN', justify='center', cursor='hand2')
    donate_label.place(relx=0.1, rely=0.95, relwidth=0.8)
    donate_label.configure(text_color=ctk.ThemeManager.theme.get('URL').get('text_color'))
    donate_label.bind('<Button>', lambda event: webbrowser.open('https://t.me/zaitian_hk'))
...
def render_video_preview(video_path: str, size: Tuple[int, int], frame_number: int = 0) -> ctk.CTkImage:
    # Use DirectShow directly by specifying the backend in VideoCapture
    capture = cv2.VideoCapture(cv2.CAP_DSHOW + video_path)
    if frame_number:
...
def webcam_preview():
    ...
    # Use DirectShow directly by specifying the backend in VideoCapture
    cap = cv2.VideoCapture(cv2.CAP_DSHOW + 0)  # Use index for the webcam with DirectShow backend   
